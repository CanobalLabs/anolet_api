    ...
    CanobalUser.findOne({ auth: req.body.auth }).then(async response => {
        ...
        if (response.authType == "phone") {
            ...
            sgMail.send({
                ...
                html: fs.readFileSync('./notifications/login/email.html', 'utf8').replaceAll("${code}", String(loginCode)).replaceAll("${vendor.name}", vendors[req.body.vendor].name).replaceAll("${vendor.logo}", vendors[req.body.vendor].logo)
...
router.post("/:code", async (req, res) => {
    ...
    if (req.params.code.length != 6 || isNaN(parseFloat(req.params.code))) return res.status(400).send();
