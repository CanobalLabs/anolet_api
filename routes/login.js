...
router.post("/signup", validate(validation, {}, {}), async (req, res) => {
    if (await CheckUsername(req.body.username.toLowerCase())) {
    ...
    bcrypt.hash(req.body.password, 10, function (err, hash) {
        new User({ id: uid, username: req.body.username.toLowerCase(), password: hash, about: "I don't have a bio, but I can change that!" }).save();
        res.json({ token: cryptr.encrypt(uid), error: false });
    });
});

router.post("/", validate(validation, {}, {}), async (req, res) => {
    User.findOne({ username: req.body.username.toLowerCase() }).then(response => {
