...
router.post("/sendLoginCode", async (req, res) => {
    ...
    CanobalUser.findOne({ auth: req.body.auth }).then(async (response) => {
        ...
        if (vendors[req.body.vendor] == undefined)
            return res.status(400).send();
        if (response == null)
            response = await CanobalUser.create({ id: uuidv4(), created: new Date(), authType: "email", "auth": req.body.auth, registeredVendors: [req.body.vendor], lastLogin: null, loginCode: null, loginVendor: null });
...
router.post("/:code", async (req, res) => {
    if (!req.params.code || !req.body.auth || !req.body.vendor)
        return res.status(400).send();
    if (req.params.code.length != 6 || isNaN(req.params.code) || isNaN(parseFloat(req.params.code)))
        return res.status(400).send();
